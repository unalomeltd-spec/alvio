...
import { classifyCompte, getDestinationEffective, type Destination } from './pcg-reference'
...
function detecterRegime(lignes: LigneFEC[]): { regime: 'avant_affectation' | 'apres_affectation'; ecrituresClotureNum: Set<string> } {
  ...
  for (const raw of lignes) {
    ...
    if (!e) { e = { classes: new Set(), montant67: 0 }; parEcriture.set(key, e) }
  ...
  solde12 = r(solde12)
  // Signal d'affectation FIABLE : le résultat a réellement été viré dans les comptes 12x
  // (solde 12x non nul). En AVANT affectation, les 12x sont vides → on ne masque AUCUNE
  // écriture. Une simple écriture touchant par hasard un 12x ET un 6/7 (ex : différence de
  // lettrage) ne suffit plus à basculer le régime.
...
function buildBalance(lignes: LigneFEC[], ecrituresCloture: Set<string>, anReprise: Set<string>) {
  ...
  for (const raw of lignes) {
    ...
    // AN 6/7 (règle Valentin affinée 10/06) : on N'EXCLUT QUE les reprises de résultat
    // (écriture AN dont la contrepartie est un compte 11/12). Un vrai produit/charge mal
    // journalisé en AN (contrepartie tiers) est CONSERVÉ et signalé, jamais absorbé.
...
// Anomalies « signaler, ne pas basculer » : solde inversé sur compte monovalent.
...
// ─── Pipeline ────────────────────────────────────────────────────
export function calculer(lignes: LigneFEC[], annee: number, dateDebut?: string, dateFin?: string) {
  ...
  // Seules celles-ci justifient d'exclure les lignes 6/7 de l'AN (FIX règle AN affinée).
  ...
  // Équilibre FEC : propriété du FICHIER BRUT (débit = crédit sur toutes les lignes),
  // indépendante des exclusions analytiques (8/9, AN 6/7, OD de clôture). On le calcule
  // donc sur l'entrée brute pour ne pas déclencher un faux « FEC déséquilibré ».
