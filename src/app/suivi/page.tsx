...
export default function SuiviPage() {
  ...
  const add = async () => {
    ...
    setSaving(true)
    await sb.from('suivi_demandes').insert({ titre, description: desc || null, auteur, destinataire: dest, priorite: prio, statut: 'à faire' })
    setTitre(''); setDesc(''); setPrio('Normal'); setFormOpen(false)
    await load()
    setSaving(false)
  }
  ...
  const visible = (view === 'archive' ? archives : actifs).filter(i => {
    ...
    if (filter === 'à faire') return i.statut === 'à faire'
    if (filter === 'en cours') return i.statut === 'en cours'
    if (filter === 'Jeremy') return i.auteur === 'Jeremy'
    if (filter === 'Val') return i.auteur === 'Val'
    return true
  })

  const prioBg: Record<Priorite, string> = { Urgent: 'rgba(216,90,48,0.1)', Normal: 'transparent', Faible: 'transparent' }
  const prioColor: Record<Priorite, string> = { Urgent: '#993C1D', Normal: '#8C9BAB', Faible: '#8C9BAB' }
          ...
          {formOpen && (
            <div style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: '16px 20px', marginBottom: 20 }}>
              ...
              <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="Description (optionnel)"
                style={{ width: '100%', border: '0.5px solid rgba(0,0,0,0.15)', borderRadius: 8, padding: '8px 12px', fontSize: 13, fontFamily: 'inherit', marginBottom: 8, outline: 'none', resize: 'vertical', minHeight: 60, color: '#1A1A1A' }} />
              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                {(['Jeremy', 'Val'] as const).map(a => (
                  <button key={a} onClick={() => setAuteur(a)}
                    style={{ flex: 1, padding: '7px', border: `0.5px solid ${auteur === a ? '#1A1A1A' : 'rgba(0,0,0,0.12)'}`, borderRadius: 8, background: auteur === a ? '#1A1A1A' : '#fff', color: auteur === a ? '#fff' : '#8C9BAB', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
                    De {a}
                  </button>
                ))}
                {(['Jeremy', 'Val', 'Les deux'] as const).map(d => (
                  <button key={d} onClick={() => setDest(d)}
                    style={{ flex: 1, padding: '7px', border: `0.5px solid ${dest === d ? '#B8A98A' : 'rgba(0,0,0,0.12)'}`, borderRadius: 8, background: dest === d ? 'rgba(184,169,138,0.12)' : '#fff', color: dest === d ? '#B8A98A' : '#8C9BAB', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
                    → {d}
                  </button>
                ))}
                {(['Normal', 'Urgent', 'Faible'] as const).map(p => (
                  <button key={p} onClick={() => setPrio(p)}
                    style={{ flex: 1, padding: '7px', border: `0.5px solid ${prio === p ? '#1A1A1A' : 'rgba(0,0,0,0.12)'}`, borderRadius: 8, background: prio === p ? '#1A1A1A' : '#fff', color: prio === p ? '#fff' : '#8C9BAB', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
                    {p}
                  </button>
                ))}
              </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
            <div style={{ display: 'flex', gap: 4 }}>
              {(['actif', 'archive'] as const).map(v => (
                <button key={v} onClick={() => { setView(v); setFilter('tous') }}
              ...
              {visible.map(item => (
                <div key={item.id} style={{ background: prioBg[item.priorite] || '#fff', border: `0.5px solid ${item.priorite === 'Urgent' ? 'rgba(216,90,48,0.2)' : 'rgba(0,0,0,0.06)'}`, borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'flex-start', gap: 12, background: '#fff' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 500, color: item.statut === 'fait' ? '#8C9BAB' : '#1A1A1A', textDecoration: item.statut === 'fait' ? 'line-through' : 'none', marginBottom: 4 }}>
                      {item.titre}
                    </div>
                    {item.description && (
                      <div style={{ fontSize: 12, color: '#8C9BAB', marginBottom: 5, lineHeight: 1.5 }}>{item.description}</div>
                    )}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      ...
                      {item.priorite === 'Urgent' && (
                        <span style={{ fontSize: 10, fontWeight: 500, color: '#993C1D', background: 'rgba(216,90,48,0.1)', padding: '1px 7px', borderRadius: 20 }}>Urgent</span>
                      )}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                    ...
                    <button onClick={() => del(item.id)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(0,0,0,0.2)', padding: '2px 4px', fontSize: 14 }}>
                      ×
                    </button>
                  </div>
                </div>
